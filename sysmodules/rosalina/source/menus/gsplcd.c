...
Menu Brightness = {
    ...
    .nbItems = 1,
    {
        { "Establecer brillo de pantalla", METHOD, .method = &setup_Brightness },
    }
};
...
{
	char screen[5][32] = {"Pantalla superior","Pantalla inferior", "Ambas pantallas","Encender//Apagar superior","Encender//Apagar inferior"};
	int index = 0;
	Draw_Lock();
	...
	while(true)
	{
		...
		Draw_Lock();
		Draw_DrawString(10, 10, COLOR_TITLE, "Establecer brillo de pantalla");
		Draw_DrawString(10, 30, COLOR_WHITE, "Presiona IZQUIERDA/DERECHA para cambiar brillo");
		Draw_DrawString(10, 40, COLOR_WHITE, "Presiona ARRIBA/ABAJO cambiar opciones");
		Draw_DrawString(10, 50, COLOR_WHITE, "Presiona B para regresar");
		Draw_DrawFormattedString(10, 70, COLOR_WHITE, "Brillo pantalla superior : %d",BrightnessTop);
		Draw_DrawFormattedString(10, 80, COLOR_WHITE, "Brillo pantalla inferior : %d",BrightnessBottom);
		for(int i = 0; i < 5; i++)
		{
