...
bool ShowUnlockSequence(u32 seqlvl) 
{
	...
	const int seqcolors[7] = { COLOR_WHITE, COLOR_GREEN, COLOR_RED,
        COLOR_TITLE, COLOR_WHITE, COLOR_RED, COLOR_RED };
	...
	u32 color_off = COLOR_WHITE;
    u32 color_on = seqcolors[seqlvl];
    u32 lvl = 0;
	
	Draw_Lock();
    Draw_DrawString(160 - ((24 * 8) / 2), 50, COLOR_WHITE, "introduce la secuencia de abajo");
	Draw_DrawString(160 - ((21 * 8) / 2), 60, COLOR_WHITE, "o presiona B para cancelar.");
	Draw_DrawString(160 - ((23 * 8) / 2), 80, COLOR_WHITE, "Para continuar, introduce esto:");
    Draw_FlushFramebuffer();
	Draw_Unlock();
    while (true) {
        for (u32 n = 0; n < len; n++) {
