...
void detectAndProcessExceptionDumps(void)
{
    ...
    drawString(true, 10, 10, COLOR_RED, "Una excepcion ha ocurrido");
    u32 posY;
    ...
    if(dumpHeader->type == 3 && (mode == 7 || mode == 11))
        posY = drawString(true, 10, posY + SPACING_Y, COLOR_YELLOW, "Dump incorrecto: fallo en dumpear codigo y/o stack") + SPACING_Y;
    ...
    static const char *choiceMessage[] = {"Presiona A para guardar el crashdump", "Presiona cualquier otro boton para apagar"};
    ...
    {
        posY = drawString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Puedes encontrar el dump en el siguiente archivo:");
        posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "%s:/luma/%s", isSdMode ? "SD" : "CTRNAND", path) + SPACING_Y;
    }
    else posY = drawString(true, 10, posY + SPACING_Y, COLOR_RED, "Error escribiendo el archivo dump");

    drawString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Presiona cualquier boton para apagar");
