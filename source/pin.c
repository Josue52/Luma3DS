...
void newPin(bool allowSkipping, u32 pinMode)
{
    ...
    drawString(true, 10, 10, COLOR_TITLE, "Introduce un PIN nuevo usando ABXY y el DPad");
    ...
    if(!fileWrite(&pin, PIN_FILE, sizeof(PinData)))
        error("Error escribiendo el archivo PIN");
}
...
bool verifyPin(u32 pinMode)
{
    ...
    drawString(true, 10, 10, COLOR_TITLE, "Introduce el PIN usando ABXY y el DPad continuar");
    drawString(true, 10, 10 + SPACING_Y, COLOR_TITLE, "Pulsa START para apagar, SELECT para limpiar");
    ...
    while(!unlock)
    {
        ...
        if(!unlock)
        {
            ...
            drawString(true, 10, 10 + 5 * SPACING_Y, COLOR_RED, "PIN equivocado, intenta de nuevo");
        }
    }
